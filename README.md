...
This project emphasizes meaningful testing over artificial coverage metrics.
...
sequenceDiagram
    ...
    participant BIO as Biometric Prompt (System)
    ...
    UI->>BIO: authenticate()
    activate BIO
    Note right of BIO: User verifies identity (Fingerprint/Face)
    BIO-->>UI: AuthResult.Success
    deactivate BIO

    UI->>VM: onAuthResult(Success)
    activate VM
    VM->>VM: Set State: Loading
    VM->>REPO: Request Passphrase Warmup
    activate REPO
    REPO->>REPO: Retrieve/Derive Key via Keystore & PBKDF2
    REPO-->>VM: (Warmup Complete)
    deactivate REPO
    
    VM->>VM: Set State: Authenticated
    ...
    REPO->>DB: queryAll()
    activate DB
    Note right of DB: SQLCipher transparently decrypts using Master Key
    DB-->>REPO: List<NoteEntity> (Decrypted in RAM)
    deactivate DB
    ...
    activate DB
    Note right of DB: SQLCipher transparently encrypts AES-256 GCM
    DB-->>REPO: Success
    deactivate DB

...
/**
 * Validates the [GetSingleNoteUseCase] domain logic.
 *
 * Uses Turbine to verify the mapping of repository flows to domain results
 * and ensures that retrieval streams are correctly closed after processing.
 */
class GetSingleNoteUseCaseTest {
    ...
    fun `invoke returns Success when note exists in flow`() = runTest {
        ...
        // When/Then
        getSingleNoteUseCase(noteId).test {
